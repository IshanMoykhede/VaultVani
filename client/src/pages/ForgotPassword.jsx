...
function ForgotPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        "http://localhost:8000/api/auth/get-reset-otp",
        { email },
        { withCredentials: true },
      );
      toast.success(res.data.message);
      navigate(`/verify-otp?email=${encodeURIComponent(email)}`); // Pass email to next page
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] text-gray-100 flex items-center justify-center px-6 py-12 relative overflow-hidden">
      {/* ... same ambient glow as SignUp/SignIn ... */}
